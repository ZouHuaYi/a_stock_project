...
def get_stock_data(stock_code, start_date, end_date):
    ...
    try:
        # 使用 Pandas 读取数据
        df = db_manager.execute_sql(query)
        if df is not None:
...
def calculate_macd(df, fast=MACD_FAST, slow=MACD_SLOW, signal=MACD_SIGNAL):
    ...
    df['MACD_hist'] = (df['DIF'] - df['DEA']) * 2 # 乘以2是常见的显示习惯
    return df
...
def check_macd_bottom_divergence(df, lookback=LOOKBACK_PERIOD_DIV):
    ...
    # 找到最近一个交易日和之前的最低点
    current_low_price = recent_data['low'].iloc[-1]
    previous_low_idx = recent_data['low'].iloc[:-1].idxmin() # 倒数第二个最低点索引
    previous_low_price = recent_data.loc[previous_low_idx, 'low']
    current_macd_hist = recent_data['MACD_hist'].iloc[-1]
    previous_low_macd_hist = recent_data.loc[previous_low_idx, 'MACD_hist']

    # 找到最近N期内的价格最低点及其索引
    abs_low_idx = recent_data['low'].idxmin()
    abs_low_price = recent_data.loc[abs_low_idx, 'low']
    abs_low_macd_hist = recent_data.loc[abs_low_idx, 'MACD_hist']

    # 条件1: 近期价格整体是下跌或探底趋势 (用简单斜率近似)
    price_trend = np.polyfit(range(len(recent_data)), recent_data['low'], 1)[0] < 0
    ...
    if price_trend and price_lower_or_near and macd_higher_low and macd_below_zero and macd_higher_than_abs_low:
        ...
        previous_low_dif = recent_data.loc[previous_low_idx, 'DIF']
...
def check_ma_break_retest(df, ma_period=YEARLY_MA_PERIOD):
    ...
    current_ma = data[f'MA_{ma_period}'].iloc[-1]
    prev_close = data['close'].iloc[-2]
    prev_ma = data[f'MA_{ma_period}'].iloc[-2]
    avg_volume_10 = data['volume'].iloc[-10:].mean()
    ...
    recent_low_below = data['low'].iloc[:-1].min() < data[f'MA_{ma_period}'].iloc[:-1].min() * 1.01

    # 条件4: 当前处于回调状态（价格比前几日低）且缩量
    is_pullback = current_close < data['high'].iloc[-5:-1].max() # 比近几日高点低
